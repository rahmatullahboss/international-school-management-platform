...
describe('CARE safeguarding domain migration contract', () => {
  test('forces tenant and case isolation and preserves append-only C4 evidence', async () => {
    const sql = await readFile(migrationPath, 'utf8');
    expect(sql).toContain('CREATE TABLE IF NOT EXISTS safeguarding.concern_intake');
    expect(sql).toContain('safeguarding.has_active_case_membership');
    expect(sql).toContain("current_setting('app.principal_id', true)");
    expect(sql).toContain("current_setting('app.purpose_code', true)");
