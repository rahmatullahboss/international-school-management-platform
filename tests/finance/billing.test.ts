...
function setup(): { billing: BillingService; ledger: LedgerService } {
  ...
  billing.assignFee({
    ...
    quantity: 2,
    adjustments: [
      { kind: 'discount', basisPoints: 1000, reason: 'Sibling discount', approvedBy: 'approver-1' },
...
describe('FIN-01 billing and receivables', () => {
  it('validates responsible-party ownership percentages', () => {
    const { billing } = setup();
    expect(() => billing.createBillingAccount({
      id: 'invalid-account',
      tenantId,
      legalEntityId,
      ...
      currency: gbp,
      status: 'active',
      responsibleParties: [{ personRef: 'guardian-3', responsibilityBasisPoints: 9000, priority: 1 }],
    }, setupPrincipal)).toThrow('FIN_RESPONSIBILITY_MUST_TOTAL_100_PERCENT');
  });

  it('generates an idempotent invoice from a fee assignment with deterministic adjustments, tax, due date and instalments', () => {
    const { billing } = setup();
    const invoice = billing.createInvoiceFromAssignment('assignment-1', '2026-07-01', setupPrincipal, 'invoice-assignment-1');
    const replay = billing.createInvoiceFromAssignment('assignment-1', '2026-07-01', setupPrincipal, 'invoice-assignment-1');
  ...
  it('splits invoice totals into stable instalments without losing a minor unit', () => {
    ...
    expect(invoice.instalments.map((instalment) => instalment.amountMinor)).toEqual([3667, 3667, 3666]);
    expect(invoice.instalments.reduce((sum, instalment) => sum + instalment.amountMinor, 0)).toBe(invoice.totalMinor);
  });

  it('posts each invoice once and traces receivable, income and tax to a balanced journal', () => {
    const { billing, ledger } = setup();
    const draft = billing.createInvoiceFromAssignment('assignment-1', '2026-07-01', setupPrincipal, 'invoice-post-proof');
  ...
  it('enforces invoice creator/poster separation and requires credit notes after posting', () => {
    const { billing } = setup();
    const creatorAndPoster = principal('same-user', ['billing.invoice.write', 'billing.invoice.post', 'ledger.journal.post']);
    ...
    expect(() => billing.voidDraftInvoice(posted.id, principal('voider', ['billing.invoice.void']))).toThrow('FIN_POSTED_INVOICE_REQUIRES_CREDIT_NOTE');
  });
  ...
  it('posts a bounded credit note, updates invoice balance and preserves bidirectional ledger trace', () => {
    ...
    const invoice = billing.postInvoice({
      invoiceId: billing.createInvoiceFromAssignment('assignment-1', '2026-07-01', setupPrincipal, 'invoice-credit-proof').id,
      periodId,
      postedBy: invoicePoster,
    ...
    expect(billing.getInvoice(invoice.id)).toMatchObject({ creditedMinor: 5_500, balanceMinor: 13_200 });
    expect(ledger.getEntriesForSource('credit-note', credit.id)[0]?.lines.map((journalLine) => [journalLine.accountId, journalLine.side, journalLine.amountMinor])).toEqual([
      ['tuition-income', 'debit', 5_000],
      ['tax-payable', 'debit', 500],
      ['receivable', 'credit', 5_500],
    ]);
    expect(() => billing.createCreditNote({
      invoiceId: invoice.id,
  ...
  it('allocates and unallocates oldest instalments while keeping invoice totals consistent', () => {
    ...
    expect(partial).toMatchObject({ status: 'partially-paid', allocatedMinor: 7_000, balanceMinor: 4_000 });
    expect(partial.instalments.map((instalment) => instalment.allocatedMinor)).toEqual([5_500, 1_500]);
    const reversed = billing.reverseAllocation(invoice.id, 2_000, setupPrincipal);
    expect(reversed).toMatchObject({ status: 'partially-paid', allocatedMinor: 5_000, balanceMinor: 6_000 });
  ...
  it('produces a reproducible as-of statement from posted invoices and credit notes', () => {
    const { billing } = setup();
    const invoice = billing.postInvoice({
      invoiceId: billing.createInvoiceFromAssignment('assignment-1', '2026-07-01', setupPrincipal, 'invoice-statement-proof').id,
      periodId,
      postedBy: invoicePoster,
    ...
    expect(billing.statement('family-account-1', '2026-07-05')).toMatchObject({ closingBalanceMinor: 18_700 });
    expect(billing.statement('family-account-1', '2026-07-31')).toMatchObject({ closingBalanceMinor: 16_500 });
    expect(billing.statement('family-account-1', '2026-07-31').entries.map((entry) => entry.type)).toEqual(['invoice', 'credit-note']);
  });
