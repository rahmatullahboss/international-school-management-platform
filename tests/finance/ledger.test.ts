...
import { LedgerService, type LedgerAccountRecord, type PostJournalCommand } from '../../packages/modules/ledger/src/index.js';
...
function principal(principalId: string, permissions: FinancePrincipal['permissions'], assurance: FinancePrincipal['assurance'] = 'aal2'): FinancePrincipal {
...
describe('FIN-01 immutable double-entry ledger', () => {
  ...
  it('rejects unbalanced, mixed-currency and cross-tenant postings', () => {
    const ledger = setup();
    expect(() => ledger.post(invoiceCommand({ lines: [
      { accountId: 'receivable', side: 'debit', amountMinor: 25001, currency: gbp },
      { accountId: 'tuition-income', side: 'credit', amountMinor: 25000, currency: gbp },
    ] }))).toThrow('FIN_UNBALANCED_JOURNAL');
    expect(() => ledger.post(invoiceCommand({ idempotencyKey: 'mixed-currency-1', lines: [
      { accountId: 'receivable', side: 'debit', amountMinor: 25000, currency: gbp },
      { accountId: 'tuition-income', side: 'credit', amountMinor: 25000, currency: currencyCode('USD') },
    ] }))).toThrow('FIN_CURRENCY_MISMATCH');
    expect(() => ledger.post(invoiceCommand({ tenantId: 'tenant-b', idempotencyKey: 'cross-tenant-1' }))).toThrow('FIN_SCOPE_MISMATCH');
  });
  ...
  it('enforces creator/poster separation and assurance', () => {
    ...
    expect(() => ledger.post(invoiceCommand({ idempotencyKey: 'low-assurance-1', postedBy: principal('low', ['ledger.journal.post'], 'aal1') }))).toThrow('FIN_STEP_UP_REQUIRED');
  });
  ...
  it('closes periods, blocks posting, and requires a different AAL3 reopener', () => {
    ...
    expect(() => ledger.reopenPeriod(periodId, principal('closer', ['ledger.period.reopen'], 'aal3'), 'Correction required')).toThrow('FIN_SOD_VIOLATION');
  ...
  it('creates one linked, balanced reversal and nets account balances to zero', () => {
    ...
    const reversal = ledger.reverse(original.id, reverser, 'Invoice cancelled', 'reverse:invoice-1');
    expect(reversal.reversalOf).toBe(original.id);
    expect(ledger.reverse(original.id, reverser, 'Invoice cancelled', 'reverse:invoice-1')).toBe(reversal);
    expect(ledger.balances().map((balance) => balance.balanceMinor)).toEqual([0, 0]);
    expect(() => ledger.reverse(original.id, principal('poster', ['ledger.journal.post', 'ledger.journal.reverse']), 'Second reversal', 'reverse:invoice-1-again')).toThrow('FIN_SOD_VIOLATION');
  });

  it('preserves historical as-of balances', () => {
    const ledger = setup();
    ledger.post(invoiceCommand({ entryDate: '2026-06-30', idempotencyKey: 'invoice-post:june', sourceDocumentId: 'invoice-june' }));
    ledger.post(invoiceCommand({ entryDate: '2026-07-28', idempotencyKey: 'invoice-post:july', sourceDocumentId: 'invoice-july' }));
    expect(ledger.listEntries('2026-06-30')).toHaveLength(1);
    expect(ledger.balances('2026-06-30').find((balance) => balance.accountId === 'receivable')?.balanceMinor).toBe(25000);
    expect(ledger.balances('2026-07-28').find((balance) => balance.accountId === 'receivable')?.balanceMinor).toBe(50000);
  });
  ...
  it('maintains balance over a deterministic high-volume property sweep', () => {
    ...
    expect(totals.find((balance) => balance.accountId === 'receivable')?.balanceMinor).toBe(2_129_250);
    expect(totals.find((balance) => balance.accountId === 'tuition-income')?.balanceMinor).toBe(2_129_250);
  });
});
