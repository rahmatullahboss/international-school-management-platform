...
Focused unit tests cover capability filtering, RTL resolution, persona separation, offline guidance, verified-session context and recoverable loading/error states. Dedicated Playwright evidence covers desktop semantics and keyboard entry, 360-pixel responsive navigation/body overflow, RTL logical borders and reduced-motion behavior. Full format, lint, boundaries, typecheck, unit/integration tests, build, browser suite, dependency/provenance checks and execution-artifact validation are required before the checkpoint is merged.
