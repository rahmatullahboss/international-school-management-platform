...
- Content works with keyboard, screen readers, narrow viewports, RTL and forced-colour modes.
...
- HTML escaping, protocol-relative rejection and unsafe URL rejection.
...
- Table semantics remain intact on desktop; mobile cells retain visible labels.
...
- All directional spacing and borders use logical properties, so the same markup supports LTR and RTL.

## Hardening

- All user/data labels are HTML escaped before rendering.
- Links that do not begin with a safe single `/` or `#` are replaced with `#`; `javascript:`, external and protocol-relative injection are rejected.
- Duplicate IDs in metrics, exceptions, queues, modules and actions are removed deterministically.
...
- State-changing commands carry correlation and idempotency context; the application façade enforces handler policy.
...
The reviewed base has no browser-automation or visual-regression harness. Evidence is semantic-render, CSS-contract, security and type/test based rather than screenshot-diff based. No browser or production deployment was performed.
