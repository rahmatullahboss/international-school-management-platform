...
void main() {
  group('school locale policy', () {
    test('supports English, Bangla and Arabic with safe fallback', () {
      expect(
        SchoolLocalePolicy.isSupported(const Locale('en', 'GB')),
        isTrue,
      );
      expect(
        SchoolLocalePolicy.isSupported(const Locale('bn', 'BD')),
        isTrue,
      );
      expect(
        SchoolLocalePolicy.isSupported(const Locale('ar', 'SA')),
        isTrue,
      );
      expect(
        SchoolLocalePolicy.isSupported(const Locale('fr', 'FR')),
        isFalse,
      );
      expect(
    ...
    test('provides localized shell copy without authority values', () {
      final bangla = SchoolShellStrings.forLocale(
        const Locale('bn', 'BD'),
      );
      final arabic = SchoolShellStrings.forLocale(
        const Locale('ar', 'SA'),
      );
      final fallback = SchoolShellStrings.forLocale(
        const Locale('fr', 'FR'),
      );
  ...
  testWidgets(
    'adaptive scaffold survives 200 percent text scaling',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(360, 800));
      addTearDown(() => tester.binding.setSurfaceSize(null));

      final strings = SchoolShellStrings.forLocale(
        const Locale('en', 'US'),
      );
  ...
  testWidgets(
    'Arabic shell follows RTL reading order and written semantics',
    (tester) async {
      final strings = SchoolShellStrings.forLocale(
        const Locale('ar', 'SA'),
      );
  ...
  testWidgets(
    'interactive controls preserve the 48 logical pixel target',
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
