...
void main() {
  test('loads teacher day with selected school scope headers', () async {
    ...
    final today = await TeacherMobileApi(client).loadToday(
      teacherSession(),
      correlationId: 'teacher-today-correlation',
    );
    ...
    expect(capturedRequest.headers['x-correlation-id'], 'teacher-today-correlation');
  ...
  test('loads a versioned roster for the requested meeting', () async {
    ...
    final roster = await TeacherMobileApi(client).loadRoster(
      meetingId: 'meeting-1',
  ...
  test('rejects teacher requests without capability before transport', () async {
  ...
  test('submits idempotent attendance batches without client finalization', () async {
    late http.Request capturedRequest;
    ...
    final receipt = await TeacherMobileApi(client).submitAttendance(
      command: command,
      session: teacherSession(),
    );
  ...
  test('saves exact integer grade drafts without publication authority', () async {
    late Map<String, Object?> capturedBody;
    final client = apiClient((request) async {
      capturedBody = jsonDecode(request.body) as Map<String, Object?>;
      return http.Response(
    ...
    final receipt = await TeacherMobileApi(client).saveGradeDraft(
      command: command,
      session: teacherSession(),
    );
    ...
    expect((entries.last as Map<String, Object?>).containsKey('scoreUnits'), isFalse);
    expect(capturedBody.containsKey('publish'), isFalse);
