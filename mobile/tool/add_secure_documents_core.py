...
write(
    "packages/secure_documents/pubspec.yaml",
    r'''
name: school_secure_documents
description: Step-up authenticated no-store document exchange for School mobile apps.
version: 0.1.0
publish_to: none
...
dependencies:
  ...
  http: ^1.6.0
  path_provider: ^2.1.6
  school_authentication: ^0.1.0
  school_family_domain: ^0.1.0
...
write(
    "packages/secure_documents/lib/school_secure_documents.dart",
    r'''
library;
...
final class HttpSecureDocumentTransport implements SecureDocumentTransport {
  ...
  Future<SecureDocumentStreamResponse> exchange({
    ...
    return SecureDocumentStreamResponse(
      ...
      mediaType: mediaType,
      noStore: cacheControl.split(',').any(
        (directive) => directive.trim() == 'no-store',
      ),
      sha256Hex: digest,
    );
  }
  ...
  static Uri _normalizeBaseUri(Uri value) {
    if (value.scheme != 'https' || value.host.isEmpty) {
      throw const SecureDocumentException(
        'SECURE_DOCUMENT_API_HTTPS_REQUIRED',
      );
    }
...
final class _FileNoStoreDocumentLease implements SecureDocumentLease {
  ...
  void _ensureWritable() {
    if (_sealed || _disposed) {
      throw const SecureDocumentException(
        'SECURE_DOCUMENT_LEASE_NOT_WRITABLE',
      );
    }
  }
