...
write(
    "packages/secure_documents/test/secure_documents_test.dart",
    r'''
import 'dart:convert';
...
void main() {
  test('uses step-up, verifies integrity and disposes no-store lease', () async {
    ...
    expect(receipt.toString(), isNot(contains(sha256.convert(bytes).toString())));
...
final class _MemoryLease implements SecureDocumentLease {
  ...
  Future<void> present(
    ...
    presented = true;
    await presenter.presentFile(mediaType: mediaType, path: '[opaque-test-path]');
  }
...
replace_once(
    "pubspec.yaml",
    "  - packages/native_notifications\n",
    "  - packages/native_notifications\n  - packages/secure_documents\n",
)
replace_once(
    "apps/family_app/pubspec.yaml",
    "  school_mobile_core: ^0.1.0\n",
    "  school_mobile_core: ^0.1.0\n  school_secure_documents: ^0.1.0\n",
)
