...
def normalize_imports(source: str, *, staff: bool) -> str:
...
def wire_family() -> None:
    ...
    source = normalize_imports(path.read_text(encoding="utf-8"), staff=False)
    source = ensure_once(
        source,
        f"{MOBILE_CORE}\n",
...
def wire_staff() -> None:
    ...
    source = normalize_imports(path.read_text(encoding="utf-8"), staff=True)
    source = ensure_once(
        source,
        f"{MOBILE_CORE}\n",
        "\npart 'production_app.dart';\n",
