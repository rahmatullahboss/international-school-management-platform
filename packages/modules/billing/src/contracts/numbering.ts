...
export function createDefaultInvoiceNumberingPolicy(tenantId: string): NumberingPolicy {
  ...
  return createNumberingPolicy(
    ...
    [{ ruleId: `invoice:${tenantId}`, documentType: 'invoice', sequenceId: `invoice:${tenantId}`, priority: 100 }],
  );
}
...
export function allocateSequenceNumber(policy: NumberingPolicy, documentType: string, context: NumberingContext, idempotencyKey: string): SequenceAllocation;
export function allocateSequenceNumber(
  ...
  const idempotencyKey = typeof contextOrIdempotencyKey === 'string' ? contextOrIdempotencyKey : maybeIdempotencyKey;
  ...
  if (idempotencyKey === undefined || idempotencyKey.trim().length < 8) throw new Error('Idempotency key must contain at least 8 characters');
  ...
  if (context !== undefined) {
    if (sequence.scope === 'tenant' && context.tenantId !== sequence.scopeRef) throw new Error('Numbering scope mismatch');
    if (sequence.scope === 'legal-entity' && context.legalEntityId !== sequence.scopeRef) throw new Error('Numbering scope mismatch');
    if (sequence.scope === 'campus' && context.campusId !== sequence.scopeRef) throw new Error('Numbering scope mismatch');
  }
