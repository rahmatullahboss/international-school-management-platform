...
export function calculateTotals(lines: readonly JournalLine[]): {
  debits: number;
  credits: number;
} {
  return lines.reduce(
...
export function validateJournalEntry(
  ...
  for (const line of lines) {
    if (!Number.isSafeInteger(line.amount) || line.amount <= 0)
      errors.push(`Line ${line.lineNumber}: amount must be a positive safe integer`);
  ...
  if (totals.debits !== totals.credits)
    errors.push(`Entry is not balanced: debits=${totals.debits}, credits=${totals.credits}`);
...
export function isBalanced(lines: readonly JournalLine[]): boolean {
  ...
  return (
    ...
    lines.every((line) => Number.isSafeInteger(line.amount) && line.amount > 0)
  );
}
...
export function createReversalEntry(
  ...
  reason: string,
  now = new Date(),
): { entry: JournalEntry; lines: readonly JournalLine[] } {
