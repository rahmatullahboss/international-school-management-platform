...
CREATE TABLE IF NOT EXISTS safeguarding.concern_status_event (
  ...
  status text NOT NULL CHECK (status IN ('received','linked-to-case','closed-no-action')),
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'concern_intake','concern_status_event','chronology_entry','assessment','safety_plan',
    'mandatory_report','mandatory_report_status_event','external_disclosure',
...
GRANT SELECT, INSERT ON safeguarding.safety_plan TO app_runtime;
...
VALUES (
  ...
  'CARE-01',
  'Write-only concern intake, existence-masked safeguarding cases, immutable chronology and exact disclosures'
)
