...
export const CARE_API_V1_ROUTES = [
  {
...
] as const satisfies readonly CareApiRoute[];

export type CareApiRouteId = (typeof CARE_API_V1_ROUTES)[number]['id'];
...
export function authorizeCareApiRoute(input: {
  ...
  const route = CARE_API_V1_ROUTES.find((candidate) => candidate.id === input.routeId);
  if (!route) {
    return {
      allowed: false,
      status: 404,
      code: 'CARE_API_ROUTE_NOT_FOUND',
      masked: true,
    };
  }
  if (!input.context.tenantId || !input.context.principalId) {
    return {
      allowed: false,
      status: route.masksExistence ? 404 : 401,
      code: 'CARE_API_CONTEXT_REQUIRED',
      masked: route.masksExistence,
    };
  }
  if (!input.context.membershipActive) {
    return {
      allowed: false,
      status: route.masksExistence ? 404 : 403,
      code: 'CARE_API_MEMBERSHIP_INACTIVE',
      masked: route.masksExistence,
    };
  }
  if (!input.context.permissions.includes(route.permission)) {
    return {
      allowed: false,
      status: route.masksExistence ? 404 : 403,
  ...
  if (!input.context.purpose || !route.purposes.includes(input.context.purpose)) {
    return {
      allowed: false,
      status: route.masksExistence ? 404 : 403,
  ...
  if (route.assurance === 'aal2' && input.context.assurance !== 'aal2') {
    return {
      allowed: false,
      status: 403,
