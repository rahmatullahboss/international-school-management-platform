...
import type {
  TeacherRuntimeProjectionComposition,
  TeacherRuntimeProjectionCompositionInput,
  TeacherRuntimeProjectionCompositionRejectionReason,
  TeacherRuntimeProjectionCompositionResult,
} from './runtime-teacher-projection-composer.js';

interface TeacherCompositionRow extends Record<string, unknown> {
...
const REJECTION_REASONS = new Set<TeacherRuntimeProjectionCompositionRejectionReason>([
  'invalid-composition',
  'scope-inactive',
  'persona-not-teacher',
  'staff-unlinked',
...
function invalidResponse(): Error {
  return new Error('Teacher runtime projection composer returned an invalid database response.');
}

function validateComposition(value: unknown): TeacherRuntimeProjectionComposition {
  ...
  return {
    ...
    state: value.state as TeacherRuntimeProjectionComposition['state'],
...
function validateResult(value: unknown): TeacherRuntimeProjectionCompositionResult {
  ...
  if (
    ...
    !REJECTION_REASONS.has(value.reason as TeacherRuntimeProjectionCompositionRejectionReason) ||
...
export class DatabaseTeacherProjectionComposerStore {
  ...
  async compose(
    ...
    const rows = await this.#database.query<TeacherCompositionRow>(
      `SELECT platform.compose_teacher_runtime_projection_source(
         $1::uuid,
         $2::uuid,
         $3::uuid,
         $4::bigint,
