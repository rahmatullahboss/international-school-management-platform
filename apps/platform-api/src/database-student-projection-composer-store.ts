...
import type {
  StudentRuntimeProjectionComposition,
  StudentRuntimeProjectionCompositionInput,
  StudentRuntimeProjectionCompositionRejectionReason,
  StudentRuntimeProjectionCompositionResult,
} from './runtime-student-projection-composer.js';

interface StudentCompositionRow extends Record<string, unknown> {
...
const REJECTION_REASONS = new Set<StudentRuntimeProjectionCompositionRejectionReason>([
  'invalid-composition',
  'scope-inactive',
  'persona-not-student',
  'student-unlinked',
...
function invalidResponse(): Error {
  return new Error('Student runtime projection composer returned an invalid database response.');
}

function validateComposition(value: unknown): StudentRuntimeProjectionComposition {
  ...
  return {
    ...
    state: value.state as StudentRuntimeProjectionComposition['state'],
...
function validateResult(value: unknown): StudentRuntimeProjectionCompositionResult {
  ...
  if (
    ...
    !REJECTION_REASONS.has(value.reason as StudentRuntimeProjectionCompositionRejectionReason) ||
...
export class DatabaseStudentProjectionComposerStore {
  ...
  async compose(
    ...
    const rows = await this.#database.query<StudentCompositionRow>(
      `SELECT platform.compose_student_runtime_projection_source(
         $1::uuid,
         $2::uuid,
         $3::uuid,
         $4::bigint,
