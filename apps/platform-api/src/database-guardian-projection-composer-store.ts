...
import type {
  GuardianRuntimeProjectionComposition,
  GuardianRuntimeProjectionCompositionInput,
  GuardianRuntimeProjectionCompositionRejectionReason,
  GuardianRuntimeProjectionCompositionResult,
} from './runtime-guardian-projection-composer.js';

interface GuardianCompositionRow extends Record<string, unknown> {
...
const REJECTION_REASONS = new Set<GuardianRuntimeProjectionCompositionRejectionReason>([
  'invalid-composition',
  'scope-inactive',
...
function invalidResponse(): Error {
  return new Error('Guardian runtime projection composer returned an invalid database response.');
}

function validateComposition(value: unknown): GuardianRuntimeProjectionComposition {
  ...
  return {
    ...
    state: value.state as GuardianRuntimeProjectionComposition['state'],
...
function validateResult(value: unknown): GuardianRuntimeProjectionCompositionResult {
  ...
  if (
    ...
    !REJECTION_REASONS.has(value.reason as GuardianRuntimeProjectionCompositionRejectionReason) ||
...
export class DatabaseGuardianProjectionComposerStore {
  ...
  async compose(
    ...
    const rows = await this.#database.query<GuardianCompositionRow>(
      `SELECT platform.compose_guardian_runtime_projection_source(
         $1::uuid,
         $2::uuid,
         $3::uuid,
         $4::bigint,
