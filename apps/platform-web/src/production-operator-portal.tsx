...
const operatorConfig: Readonly<
  Record<
    OperatorRole,
    {
      ...
      readonly routes: readonly { readonly href: string; readonly label: string; readonly detail: string }[];
    }
  >
> = {
  admissions: {
    title: 'Admissions workspace',
    description: 'Review enquiries, applications and enrolment work within the current school scope.',
    routes: [
      { href: '/admissions/enquiries', label: 'Enquiries', detail: 'Prospective-family enquiry workflow' },
      { href: '/admissions/applications', label: 'Applications', detail: 'Application review and evidence' },
      { href: '/admissions/interviews', label: 'Interviews', detail: 'Interview scheduling and review' },
    ],
  },
  finance: {
    ...
    routes: [
      ...
      { href: '/finance/reconciliation', label: 'Reconciliation', detail: 'Reviewed payment matching' },
    ],
  },
  support: {
    ...
    routes: [
      { href: '/support/tenants', label: 'Tenant scope', detail: 'Explicit support tenant selection' },
      { href: '/support/health', label: 'Deployment health', detail: 'Approved operational diagnostics' },
      { href: '/support/access', label: 'Privileged access', detail: 'Purpose-bound AAL2 support access' },
    ],
  },
};
  ...
  return (
    <div className="pilot-entry" data-role={role}>
      <header className="pilot-entry__masthead">
        <div>
          <p className="pilot-kicker">Ozzyl International Demo School · authenticated production QA</p>
      ...
      <main className="pilot-entry__main" id="main-content" tabIndex={-1}>
        <nav className="pilot-actions" aria-label={`${config.title} navigation`}>
          {config.routes.map((route) => (
            <a href={route.href} key={route.href} aria-current={route.href === props.pathname ? 'page' : undefined}>
              {route.label}
            </a>
          ))}
        </nav>
        <section className="pilot-demo-note">
          ...
          <span>
            Synthetic pilot sessions and synthetic operator metrics are disabled here. Domain writes remain governed by the reviewed database command contracts.
          </span>
        </section>
